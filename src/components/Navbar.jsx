...
const Navbar = () => {
  ...
  const searchTextDefaultValue = query.get("q") || "";
  const [searchText, setSearchText] = useState(searchTextDefaultValue);
  const { isLoggedIn, handleLogOutClick, cart, user } = useAuthContext();
  const navigate = useNavigate();
  const [menuOpen, setMenuOpen] = useState(false);
  const [showProfile, setShowProfile] = useState(false); // Profile modal toggle
...
{showProfile && (
  <div className="fixed inset-0 z-50 flex items-center justify-center">
    ...
    {/* Modal content */}
    <div className="relative bg-white rounded-3xl shadow-2xl p-8 w-11/12 max-w-md md:max-w-lg animate-fadeIn">
      {/* Close button */}
      <button
        onClick={() => setShowProfile(false)}
      ...
      {/* User info */}
      <h2 className="text-3xl md:text-4xl font-bold mb-4 text-center">{user?.name}</h2>
      ...
      <Link
        to="/my-orders"
        onClick={() => setShowProfile(false)}
        className="mt-6 block w-full text-center bg-blue-600 text-white py-3 rounded-xl hover:bg-blue-700 text-lg font-medium transition-all duration-200"
      >
        My Orders
      </Link>
      ...
      <button
        onClick={handleLogOutClick}
        className="mt-8 w-full bg-red-500 text-white py-3 rounded-xl hover:bg-red-600 text-lg font-medium transition-all duration-200"
      >
        Logout
      </button>
    </div>
